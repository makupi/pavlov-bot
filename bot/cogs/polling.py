...
class Polling(commands.Cog):
    ...
    async def player_polling(
        ...
        if poll_config.get("show_scoreboard"):
            ...
            scoreboard = await players_command(server)
    ...
    async def autobalance_polling(self, poll_config: dict, server: str, poll_name: str):
        ...
        teamblue, teamred, _, _, scoredict = await players.get_stats(server)
        for player, score in scoredict.items():
            ...
            if score < int(poll_config.get("tk_threshold")):
                ...
                if tk_action.casefold() == "kick":
                    _ = await exec_server_command(server, f"Kick {player}")
                    logging.info(
                    ...
                    _ = await exec_server_command(server, f"Ban {player}")
                    logging.info(
...
#                        logging.info(f"Median: {median_number}")
                ...
                if poll_config.get("autobalance_testing"):
                    ...
                    _ = await exec_server_command(server, sw_command)
                    embed = discord.Embed(
